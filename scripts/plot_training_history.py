    ...
    try:
        ...
        for key, value in lstm_results.items():
            if isinstance(value, (list, np.ndarray)):
                print(
                    f"{key}: length {len(value)}, sample: {value[:3] if len(value) > 0 else 'empty'}"
                )
            else:
                print(f"{key}: {value}")
        ...
        if "training_history" in lstm_results:
            ...
            key in lstm_results
            for key in ["train_losses", "train_accuracies", "val_losses", "val_accuracies"]
        ):
...
def plot_comparison():
    ...
    try:
        ...
        if phobert_val_acc:
            ...
            plt.plot(
                phobert_epochs, phobert_val_acc, "g-", label="PhoBERT", linewidth=2, marker="s"
            )
            has_data = True
        ...
        if "val_losses" in lstm_data and lstm_data["val_losses"]:
            ...
            plt.plot(
                lstm_epochs, lstm_data["val_losses"], "b-", label="LSTM", linewidth=2, marker="o"
            )
            has_data = True
        ...
        if phobert_val_loss:
            ...
            plt.plot(
                phobert_epochs, phobert_val_loss, "g-", label="PhoBERT", linewidth=2, marker="s"
            )
            has_data = True
        ...
        phobert_train_acc = phobert_data.get(
            "train_accuracies", phobert_data.get("train_accuracy", [])
        )
        if phobert_train_acc:
            phobert_epochs = range(1, len(phobert_train_acc) + 1)
            plt.plot(
                phobert_epochs, phobert_train_acc, "g-", label="PhoBERT", linewidth=2, marker="s"
            )
            has_data = True
        ...
        if "train_losses" in lstm_data and lstm_data["train_losses"]:
            ...
            plt.plot(
                lstm_epochs, lstm_data["train_losses"], "b-", label="LSTM", linewidth=2, marker="s"
            )
            has_data = True
        ...
        if phobert_train_loss:
            ...
            plt.plot(
                phobert_epochs, phobert_train_loss, "g-", label="PhoBERT", linewidth=2, marker="s"
            )
            has_data = True
