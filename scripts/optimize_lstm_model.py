...
from src.models.lstm_model import VietnameseTextDataset, LSTMEmotionDetector
...
class OptimizedLSTMDetector(LSTMEmotionDetector):
    ...
    def create_model(self):
        ...
        print(f"Improved model created with {sum(p.numel() for p in self.model.parameters())} parameters")
        return self.model
    ...
    def train_optimized(self, train_loader, val_loader, class_weights_dict, 
                       epochs=25, learning_rate=0.0005):
        """Optimized training with better hyperparameters"""
        print("Starting optimized training...")
        
        # Weighted loss function for class imbalance
        class_weights_tensor = torch.tensor([class_weights_dict[i] for i in range(len(class_weights_dict))], 
...
def main():
    ...
    training_history = detector.train_optimized(
        ...
        epochs=25, learning_rate=0.0005  # Lower learning rate
    )
