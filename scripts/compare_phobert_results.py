...
def create_detailed_metrics_comparison(baseline_results, finetuned_results):
    ...
    for emotion in emotions:
        ...
        metrics_data.extend([
            {'Emotion': emotion, 'Model': 'Baseline', 'Metric': 'Precision', 'Value': baseline_precision},
            {'Emotion': emotion, 'Model': 'Baseline', 'Metric': 'Recall', 'Value': baseline_recall},
            {'Emotion': emotion, 'Model': 'Baseline', 'Metric': 'F1-Score', 'Value': baseline_f1},
            {'Emotion': emotion, 'Model': 'Fine-tuned', 'Metric': 'Precision', 'Value': finetuned_precision},
            {'Emotion': emotion, 'Model': 'Fine-tuned', 'Metric': 'Recall', 'Value': finetuned_recall},
            {'Emotion': emotion, 'Model': 'Fine-tuned', 'Metric': 'F1-Score', 'Value': finetuned_f1}
        ])
...
def main():
    ...
    print(f"Baseline accuracy: {baseline_acc:.4f}" if isinstance(baseline_acc, (int, float)) else f"Baseline accuracy: {baseline_acc}")
    print(f"Fine-tuned accuracy: {finetuned_acc:.4f}" if isinstance(finetuned_acc, (int, float)) else f"Fine-tuned accuracy: {finetuned_acc}")
    ...
    fig_main.write_image('results/phobert_baseline_vs_finetuned_comparison.png', width=1200, height=800)
    
    # Detailed metrics comparison
    fig_detailed, df_metrics = create_detailed_metrics_comparison(baseline_results, finetuned_results)
    fig_detailed.write_html('results/phobert_detailed_metrics_comparison.html')
    fig_detailed.write_image('results/phobert_detailed_metrics_comparison.png', width=1200, height=600)
