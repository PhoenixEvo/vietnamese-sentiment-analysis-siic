...
def load_all_model_results():
    ...
    try:
        ...
        for _, row in baseline_df.iterrows():
            ...
            if model_name not in ['optimized_lstm', 'phobert']:  # Skip duplicates
                ...
                print(f" {model_name}: Accuracy = {row['accuracy']:.4f}, F1 = {row['f1_score']:.4f}")
    ...
    try:
        ...
        if 'test_results' in lstm_results:
            ...
            print(f" LSTM: Accuracy = {test_results['accuracy']:.4f}, F1 = {test_results['f1_score']:.4f}")
    except Exception as e:
    ...
    try:
        ...
        print(f" PhoBERT: Accuracy = {phobert_results['accuracy']:.4f}, F1 = {phobert_results['f1_score']:.4f}")
    except Exception as e:
...
def plot_model_performance_comparison(results):
    ...
    for i, model in enumerate(models):
        plt.scatter(accuracies[i], f1_scores[i], 
    ...
    bars3 = plt.bar(x - width/2, type_accuracies, width, label='Accuracy', 
                   color='lightblue', alpha=0.7, edgecolor='black')
    bars4 = plt.bar(x + width/2, type_f1s, width, label='F1-Score', 
    ...
    bars5 = plt.barh(sorted_models, sorted_accuracies, color=sorted_colors, alpha=0.7, edgecolor='black')
    ...
    plt.pie(type_counts.values(), labels=labels, colors=colors_pie, autopct='%1.1f%%',
           startangle=90)
...
def create_detailed_performance_table(results):
    ...
    print(f"{'Rank':<4} {'Model':<20} {'Type':<15} {'Accuracy':<10} {'F1-Score':<10} {'Performance'}")
    print("-" * 80)
    ...
    for i, model in enumerate(sorted_models, 1):
        ...
        print(f"{i:<4} {model:<20} {result['model_type']:<15} "
    ...
    for model_type in ['baseline', 'deep_learning', 'transformer']:
        ...
        if type_models:
            avg_acc = np.mean([results[m]['accuracy'] for m in type_models])
            avg_f1 = np.mean([results[m]['f1_score'] for m in type_models])
            print(f"   {model_type.replace('_', ' ').title():<15}: "
    ...
    for model_name, model_data in models_with_history.items():
        ...
        if 'val_losses' in history or 'val_loss' in history:
            ...
            if val_loss:
                ...
                color = '#e74c3c' if 'LSTM' in model_name else '#2ecc71'
                marker = 'o' if 'LSTM' in model_name else 's'
                plt.plot(epochs, val_loss, label=model_name, linewidth=2, marker=marker, color=color)
...
def analyze_class_performance(results):
    ...
    for i, metric in enumerate(metrics):
        ...
        for j, cls in enumerate(classes):
            plt.bar(x + j*width, class_scores[cls], width, 
                   label=cls.title(), color=colors[j], alpha=0.7)
