...
def create_performance_table(models_data):
    ...
    print("-" * 100)
    print(f"{'Model':<20} {'Accuracy':<10} {'F1-Score':<10} {'Speed (ms)':<12} {'Size (MB)':<12} {'Rank':<6}")
    print("-" * 100)
    ...
    for i, (model_name, data) in enumerate(sorted_models):
        ...
        print(f"{model_name:<20} {accuracy_pct:<9.2f}% {f1_pct:<9.2f}% {data['speed_ms']:<11} {data['model_size_mb']:<11} #{rank}")
...
def create_comparison_charts(models_data):
    ...
    for i, (name, acc, speed, size) in enumerate(zip(model_names, accuracies, speeds, sizes_scaled)):
    ...
    # Add annotations
    for i, (name, acc, speed) in enumerate(zip(model_names, accuracies, speeds)):
        plt.annotate(f'{acc:.1f}%', (speed, acc), xytext=(5, 5), 
