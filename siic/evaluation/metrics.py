...
def calculate_detailed_metrics(y_true, y_pred, model_name):
    ...
    per_class_report = classification_report(
        y_true, y_pred, target_names=list(EMOTION_LABELS.values()), output_dict=True
    )
...
def generate_detailed_report():
    ...
    if all_results:
        ...
        for i, (_, row) in enumerate(results_df_sorted.iterrows()):
            rank = i + 1
            print(
                f"{row['model_name']:<20} {row['accuracy']:<12.4f} {row['f1_weighted']:<12.4f} #{rank}"
            )
...
"""
    ...
    for i, result in enumerate(sorted_results):
        ...
        markdown_content += (
            f"| {result['model_name']} | {accuracy_pct:.2f}% | {f1_pct:.2f}% | #{rank} |\n"
        )
