...
from config.config import EMOTION_LABELS, MODELS_DIR, RESULTS_DIR
...
def comprehensive_evaluation():
    ...
    if baseline_df is not None:
        for _, row in baseline_df.iterrows():
            ...
            size_map = {"Svm": 1.6, "Random Forest": 49, "Logistic Regression": 0.118}

            # Speed estimates
            speed_map = {"Svm": 30, "Random Forest": 35, "Logistic Regression": 20}
...
def create_performance_table(results):
    ...
    for i, (model_name, data) in enumerate(sorted_results):
        ...
        print(
            f"{model_name:<18} {accuracy:<9.2f}% {f1_score:<9.2f}% {precision:<9.2f}% {recall:<9.2f}% {speed:<8} {size:<8} #{rank}"
        )
...
def create_comparison_charts(results):
    ...
    bars1 = ax1.bar(
        x - width / 2, accuracies, width, label="Accuracy (%)", alpha=0.8, color="skyblue"
    )
    bars2 = ax1.bar(
        x + width / 2, f1_scores, width, label="F1-Score (%)", alpha=0.8, color="lightcoral"
    )
