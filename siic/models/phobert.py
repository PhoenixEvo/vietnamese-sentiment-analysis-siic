...
class PhoBERTEmotionDetector:
    ...
    def prepare_data(self, data_path=None):
        ...
        if "emotion" in df.columns and "text" in df.columns:
            ...
            raise ValueError(
                f"Cannot find appropriate text and label columns in {list(df.columns)}"
            )
    ...
    def create_data_loaders(self, train_df, val_df, test_df, batch_size=16):
        ...
        train_dataset = VietnameseEmotionDataset(
            train_df["text"], train_df["label_id"], self.tokenizer, self.max_length
        )

        val_dataset = VietnameseEmotionDataset(
            val_df["text"], val_df["label_id"], self.tokenizer, self.max_length
        )

        test_dataset = VietnameseEmotionDataset(
            test_df["text"], test_df["label_id"], self.tokenizer, self.max_length
        )
    ...
    def create_model(self):
        ...
        self.model = PhoBERTEmotionClassifier(
            model_name=self.model_name, num_classes=len(self.emotion_labels), dropout=0.3
        )
    ...
    def predict_emotion(self, text):
        ...
        return {"emotion": predicted_emotion, "confidence": confidence, "probabilities": prob_dict}
    ...
    def load_model(self, filepath=None):
        ...
        self.model = PhoBERTEmotionClassifier(
            model_name=self.model_name, num_classes=len(self.emotion_labels)
        )
        ...
        try:
            ...
            if "position_ids" in str(e):
                ...
                filtered_state_dict = {
                    k: v for k, v in state_dict.items() if "position_ids" not in k
                }
