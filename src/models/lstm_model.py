...
class LSTMEmotionDetector:
    ...
    def load_model(self, filepath=None):
        ...
        if filepath is None:
            # Priority order: complete -> improved -> standard
            possible_files = [
                'improved_lstm_complete.pth',
                'improved_lstm_emotion_model.pth', 
                'lstm_emotion_model.pth'
            ]
            
            filepath = None
            for filename in possible_files:
                test_path = os.path.join(MODELS_DIR, filename)
                if os.path.exists(test_path):
                    filepath = test_path
                    print(f"✅ Found LSTM model: {filename}")
                    break
        ...
        if isinstance(checkpoint, dict) and 'vocab' in checkpoint:
            ...
            print("⚠️ Loading state_dict only - using fallback configuration")
            ...
            try:
                ...
                if os.path.exists(vocab_path):
                    ...
                    if 'embedding.weight' in state_dict:
                        ...
                        print(f"⚠️ Created minimal vocabulary with {vocab_size} tokens")
                    else:
                        self.vocab = {'<PAD>': 0, '<UNK>': 1}
                        print("⚠️ Using minimal vocabulary - predictions may be limited")
            except:
                self.vocab = {'<PAD>': 0, '<UNK>': 1}
                print("⚠️ Using minimal vocabulary fallback")
            ...
            print(f"Inferred model config from state_dict")
        ...
        if is_optimized:
            ...
            if 'fc3.weight' in state_dict and 'layer_norm.weight' in state_dict:
                print("Loading Custom Optimized LSTM model...")
                ...
                print("Loading Standard Optimized LSTM model...")
        ...
        model_type = "Optimized" if is_optimized else "Standard"
        print(f"{model_type} model loaded from: {filepath}")
...
def main():
    ...
    # Prepare data
    data_path = os.path.join(PROCESSED_DATA_DIR, 'processed_uit_vsfc_data.csv')
